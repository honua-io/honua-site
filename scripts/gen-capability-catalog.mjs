...
function renderLifecycle(cap) {
  if (!["preview", "experimental"].includes(cap.status)) return "";
  const label = cap.status === "preview" ? "Preview" : "Experimental";
  return `<p class="cap-gaps"><strong>${label}:</strong> ${esc(cap.statusNote ?? "Evaluation only; no GA commitment.")}</p>`;
}
...
function renderCategory(policy, category, caps) {
  const rows = caps.map((cap) => {
    ...
    return [
      ...
      `              <td><p class="cap-summary">${esc(cap.summary)}</p>${renderLifecycle(cap)}${renderGapsList(cap)}</td>`,
...
function renderCatalog(policy) {
  ...
  return [
    ...
    `      <p class="prov">Evidence snapshot · ${esc(policy.generatedAt)} · <a href="data/capabilities.v1.json">machine-readable catalog</a>. Edition and lifecycle wording follows the <a href="pricing.html">2026.1 commercial boundaries</a>; evidence counts do not imply GA.</p>`,
...
<html lang="en">
  ...
  <body>
    ...
    <main id="main-content" tabindex="-1" class="bed-prose">
      ...
      <p>${esc(cap.summary)}</p>${renderLifecycle(cap)}
