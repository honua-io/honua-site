...
require_fixed "Fleet promotion and verification</td><td>Planned" "$architectures"
require_fixed "Cloud Marketplace install</td><td>Planned" "$architectures"
...
require_fixed "Risky and irreversible changes remain approval-gated" "$belief"
require_fixed "operator loop is private beta" "$index"
require_fixed "fleet-wide control remains planned" "$ai_gis"
require_fixed "verify outcomes after each action" "$ai_gis"
require_fixed "does not claim unattended fleet self-healing" "$pricing"
require_fixed "verified rollout across multiple environments still planned" "$interoperability"
