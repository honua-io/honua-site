...
For the complete site, including generated `/samples/` routes, run
`./scripts/build-dist.sh` and then `python3 -m http.server --directory dist`.
Serving the repository root directly is only suitable for authored root-page
work that does not depend on generated output.
...
- Build deployable artifact: `./scripts/build-dist.sh`
  - Wipes and recreates `dist/`, copies all root `*.html`, SDK machine-doc
    ...
    into `dist/`, then generates `/samples/` and mapped legacy route shells
    from the pinned SDK consumer handoff.
- Validate the SDK gallery consumer: `node scripts/sdk-gallery-consumer.mjs --check`
  - Verifies producer hashes, content bindings, freshness, filters, route
    resolution, qualification truth, and the realtime Incident Operations
    boundary. Run `node --test scripts/sdk-gallery-consumer.test.mjs` for its
    static suite and run the browser smoke against a built `dist/` artifact.
...
There is **no third-party test framework, linter, or formatter** configured in
the repo. Focused tests use Node's built-in test runner; the gallery browser
contract launches the Chrome/Chromium already present on the CI runner.
...
- **Build output `dist/` is git-ignored** and regenerated by `build-dist.sh`.
  Do not commit it.
- **Authored pages remain root-level `*.html`** (`-maxdepth 1`). Do not author
  or commit `dist/`; `build-dist.sh` generates the nested `/samples/` catalog
  and handoff-resolved legacy route shells after copying root pages.
